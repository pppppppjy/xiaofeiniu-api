...
router.delete('/:cid',(req,res)=>{
    //注意:删除菜品类别之间必须先把属于该类别的菜品的类别编号设置为NULL
 ...
 /**
 *API: POST /admin/category 冪等/非冪等
 *请求主体参数:{cname:'xxx'}(json格式)
 *含义:添加新的菜品类别
 *返回值形如:
 * {code:200,msg:'1 categroy added',cid:x}
 */
router.post('/',(req,res)=>{
  ...
  /**
 *API: PUT /admin/category   //PATCH 
 *请求主体参数:{cid:xx,cname:'xxx'}(json格式)
 *含义:根据菜品类别编号修改该类别
 *返回值形如:
 * {code:200,msg:'1 categroy modified'}
 * {code:400,msg:'0 categroy modified,not exists'}//类别不存在
 * {code:401,msg:'0 categroy modified,no modification'}//类别存在
 */
 router.put('/',(req,res)=>{
